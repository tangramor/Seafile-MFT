...
本测试环境已内置一套「默认配对」：由 `setup.sh` 输出的内/外网仓库 ID 经 `seed_default_repo_pair` 自动写入数据库，即内网「文件源」↔ 外网「对外文件发布」。在此之上可扩展两类能力：

### 配对仓库（Repo Pairs）
- 在「配对仓库管理」界面可新增配对：填写名称后，系统在内/外网 Seafile 各确保存在一个同名仓库（不存在则自动创建）。
- 若你已分别在两个 Seafile 上建有现成仓库，创建时可直接填入两个仓库 ID，系统复用而不再自动创建；两个 ID 必须同时填写，只填一个会报错。
- 每个配对可单独启用/停用，停用的配对不参与轮询与 Webhook。
- 文件在**哪个内网配对仓库**被检测或 Web 上传，审批通过后即落到**对应的外网配对仓库**，实现按业务隔离。

### 用户分组（User Groups）
- 非管理员用户可归入分组，每个分组绑定一组配对仓库。
- 成员在上传页只能看到本分组绑定的配对仓库；审核看板、下载等页面也仅展示其可见配对相关的任务。管理员与审核者可见全部。
- 未被分配到任何分组的非管理员用户，上传页会提示「您当前未被分配到任何仓库分组，无法上传文件」。

> 清理提示：测试残留（如分组 `grp-A`、用户 `alice`/`alice123`、配对 `test-pair-1` 及其内外网同名仓库）可经界面删除；删除配对**不会**删除 Seafile 实际仓库。
...
### 测试 6：用户管理 — 编辑、重置密码、删除

**验证目标**：管理员可编辑用户的显示名、邮箱、角色，重置密码及删除本地用户。
...
**验证目标**：用户可修改自己的密码；管理员可重置其他用户密码及删除本地用户。
...
3. 确认操作按时间倒序显示：
   ...
   - 审批通过/拒绝
   - 文件传输操作
   - 用户管理变更（创建、编辑、启用/禁用、重置密码、删除）
4. 使用筛选标签查看特定操作类型（全部、任务、用户、认证）
5. 验证分页功能（上页/下页）
...
- ✅ 筛选标签正确过滤操作类型
- ✅ 分页功能正常（每页 30 条）
- ✅ 每条记录包含时间、操作者、操作、目标、详情

---

### 测试 13：多语言切换

**验证目标**：UI 根据浏览器语言或手动选择正确切换中英文。

**步骤**：

1. 使用默认 `Accept-Language: zh-CN` 的浏览器打开 MFT → UI 应显示中文
2. 将浏览器语言偏好改为英文（或使用 `?lang=en` 参数） → UI 应切换为英文
3. 确认所有页面在两种语言下均可正确渲染：
   - 登录页、首页、上传、我的申请、审核看板、下载、修改密码、用户管理、审计日志
4. 使用 URL 参数 `?lang=zh` 或 `?lang=en` 手动切换语言
5. 确认语言 Cookie 在页面导航间保持

**验证点**：
- ✅ 自动检测浏览器语言（Accept-Language 头）
- ✅ `?lang=en` 和 `?lang=zh` 参数可覆盖自动检测
- ✅ 所有 UI 文本已翻译（导航、表单、按钮、消息、提示框）
- ✅ 错误消息、邮件模板、HTTPException 详情均已翻译
- ✅ 语言偏好通过 Cookie 持久化
...
**验证目标**：审核通知邮件同时发送给 `REVIEWER_EMAILS` 配置和数据库中审核角色的用户。

**步骤**：

1. 确保 `.env` 中 `REVIEWER_EMAILS` 包含至少一个邮箱（如 `reviewer@example.com`）
2. 在数据库中创建一个角色为 "reviewer" 且邮箱有效的用户
3. 上传文件到内网 Seafile，等待 MFT 检测
4. 查看 MFT 邮件发送日志：
```bash
docker compose logs seafile-mft 2>&1 | grep -i "email\|审核邮件" | tail -10
```
5. 确认 `REVIEWER_EMAILS` 地址和 DB 审核者邮箱均收到通知
...
**验证目标**：审核看板中鼠标悬停可查看审批备注信息。

**步骤**：

1. 确保有已通过/已拒绝且含审批备注的任务
2. 进入「审核看板」
3. 找到有审批备注的任务记录
4. 鼠标悬停在状态旁的备注图标上
5. 确认提示框显示完整的审批备注内容
6. 滚动页面，确认提示框始终可见（固定定位）
7. 确认提示框不会被筛选标签或表格边缘遮挡
...
### 测试 17：配对仓库管理（自动创建 + 复用现成 ID）

**验证目标**：管理员可新建配对仓库，支持"按名称自动创建"与"填入已有仓库 ID 复用"两种方式，并验证停用/删除与校验。

**步骤**：

1. 以 admin 登录 MFT，进入「配对仓库管理」
2. 方式 A（自动创建）：填写名称如 `财务-合同`，点击「创建配对」
3. 登录内/外网 Seafile，确认各出现一个名为 `财务-合同` 的资料库
4. 方式 B（复用现成 ID）：在创建表单的「内网仓库 ID（可选）」「外网仓库 ID（可选）」分别填入两个**已存在**的仓库 ID，点击创建
5. 确认新配对出现在列表，且列表正确显示填入的仓库 ID
6. 校验分支：只填一个 ID 提交 → 应提示「请同时填写内网与外网仓库 ID，或都留空由系统自动创建」
7. 校验分支：填入不存在的 ID → 应提示「内网仓库 ID「…」不存在」
8. 对某个配对点击「停用」，确认其不再参与轮询（日志中该配对退出监控）
9. 删除测试用配对

**验证点**：
- ✅ 自动创建方式在两个 Seafile 各生成同名仓库
- ✅ 复用现成 ID 方式不新建仓库，直接使用给定 ID
- ✅ 只填一个 ID / 填不存在 ID 均被拦截并给出明确提示
- ✅ 停用后该配对退出轮询；删除配对不删除 Seafile 实际仓库

---

### 测试 18：用户分组可见性隔离

**验证目标**：将非管理员用户归入分组并绑定配对仓库后，成员只能看到本分组的仓库内容。

**步骤**：

1. 以 admin 登录，进入「配对仓库管理」，确认至少存在两个配对（如 `默认配对` 与 `财务-合同`）
2. 进入「用户分组管理」，新建分组 `grp-A`
3. 为 `grp-A` 分配配对：仅勾选 `财务-合同`，保存
4. 在「用户管理」创建一个普通用户 `alice`（角色 submitter）
5. 回到分组 `grp-A`，将 `alice` 添加为成员并保存
6. 以 `alice` 登录，进入「上传文件」
7. 确认配对下拉中**只显示** `财务-合同`，不显示 `默认配对`
8. 用 `alice` 上传一个文件到 `财务-合同`
9. 以 admin 登录，进入「审核看板」，确认能看到该任务（管理员可见全部）
10. 以另一个**未分组**的普通用户 `bob` 登录，确认上传页提示「您当前未被分配到任何仓库分组，无法上传文件」
